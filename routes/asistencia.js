...
// POST /api/asistencia/marcar-login { legajo, password, lat, lng, precision }
...
router.post('/marcar-login', (req, res) => {
  const { legajo, password, lat, lng, precision } = req.body;
  ...
  if (!abierta) {
    const result = db.prepare(`
      INSERT INTO asistencias
        (usuario_id, sede_id, fecha, ingreso_ts, ingreso_lat, ingreso_lng, ingreso_precision, ingreso_distancia_m, estado)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'abierta')
    `).run(usuario.id, usuario.sede_id_real, fecha, ahora, lat, lng, precision || null, distancia);
